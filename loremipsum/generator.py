...
_SAMPLE = resource_string(__name__, 'default/sample.txt').decode("ascii")
_DICTIONARY = resource_string(__name__, 'default/dictionary.txt').decode("ascii").split()
...
def _paragraphs(text):
    ...
    for line in text.splitlines():
        if line.strip():
            ...
            paragraphs.append([])
    return [_f for _f in [' '.join(p).strip() for p in paragraphs] if _f]
...
def _sentences(text):
    ...
    return [_f for _f in [s.strip() for s in sentences] if _f]
...
class Generator(object):
    ...
    def __init__(self, sample=None, dictionary=None, random=random.Random()):
    ...
    def __set_sample(self, sample):
        ...
        if chains:
            ...
            sentences = [_f for _f in [s.strip() for s in _sentences(sample)] if _f]
            ...
            paragraphs = [_f for _f in [p.strip() for p in _paragraphs(sample)] if _f]
    ...
    def generate_sentence(self, start_with_lorem=False):
        ...
        sentence_length = max(2, int(round(self.__random.normalvariate(mean, sigma))))
        ...
        for w in range(sentence_length - len(words)):
            ...
            if (previous not in self.__chains):
                ...
                previous = self.__random.choice(list(chains.intersection(starts)))
            ...
            # the word.
            chain = self.__random.choice(self.__chains[previous])
            word_length = chain[0]
            ...
            word = self.__random.choice(list(self.__dictionary[closest]))
    ...
    def generate_paragraph(self, start_with_lorem=False):
        ...
        sentences = max(2, int(round(self.__random.normalvariate(mean, sigma))))
